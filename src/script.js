...
function prepareDonutData(daily_counts, data_temp, tp1, tp2) {
  ...
  for (let hour = 0; hour < 24; hour++) {
	...
	//console.log("OK")/////////////New Humphrey
    //console.log(daily_per_hour_cold.push([1]));
    //console.log(daily_per_hour_hot.push([1]));
  }
...
function createDonut(cx, cy, hourData, global_color, inner_radius) {
...
//var avg = sum / hourData.cold.maxs.length;
    ...
    var points = jaggedDonut(cx, cy,
      inner_radius + 5,    //////////////////////donutMinimumRadius*2 donutMinimumRadius * (avg/1900.0)
      donutMaximumRadius,
      donutMinimumValue,
      donutMaximumValue,
      donutAngle, -1, hourData.hot.maxs, hourData.hot.mins) //avg as function parameter //console.log(avg);
...
function createMap(locs, data_counts, temp_data, callback){
  ...
  g.selectAll("circle").data( locs ).enter().append("circle")
    ...
    .on("mouseover", function(d, i) {
        ...
        donuts[i].svgs = createDonut(+s.attr("cx"), +s.attr("cy"), hourData, global_color[i], inner_radius[i]);
    })
    .on("click", (d, i) => {
